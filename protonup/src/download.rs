...
pub(crate) async fn download_file(
    ...
    tracing::debug!("Download file handler");
    ...
    files::download_to_async_write(
        ...
        &mut file, // TODO: ADD PROGRESS WRAPPER HERE
...
pub async fn run_quick_downloads(force: bool) -> Result<Vec<Release>> {
    println!("Running quick Downloads with force : {} ", force);
    tracing::debug!("Running quick Downloads with force : {} ", force);
...
async fn download_validate_unpack(
    ...
    tracing::debug!("download_validate_unpack");
    let download = release.get_download_info();
    // let file = download_file(&download, multi_progress.clone(), temp_dir)
    let file = download_file(&download, temp_dir).await.with_context(|| {
        format!(
